...
项目简述：针对大语言模型在开放式科学发现中的工具调用局限，本项目构建了生物信息学基准测试Co-BioBench，并提出SDTL微调框架。通过双代理协作数据生成与动态课程学习，实现了7B规模模型在复杂科学工作流上超越GPT-4o的性能。
\begin{itemize}[nosep]
    ...
    \item 针对数据构建，检索15篇开放代码的文献，基于双代理交互范式提取3785个真实R/Python工具调用任务，完成多轮数据清洗后，利用LLM自动生成负向样本（构造干扰函数及错误参数组合）以提升模型判别能力。
    \item 所构建的数据集支撑微调模型在Co-BioBench基准上取得51.65\%准确率，较基线提升17个百分点并超越GPT-4o；消融实验表明，数据增强策略显著提升了模型在多样化的科学应用中的鲁棒性，有效解决了科学工具命名歧义带来的泛化难题。
\end{itemize}
% --- 第一个项目 ---
